...
import React, { useState, useEffect, useMemo } from "react";
import api from "./api.js"; // FIX #1: shared axios instance, env-based base URL instead of hardcoded localhost
...
const EmployeeDashboard = () => {
  ...
  const [projects, setProjects] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null); // FIX #3: surface fetch failures instead of silently rendering zeros
  const [completingTaskId, setCompletingTaskId] = useState(null); // per-task loading state for the complete button
  ...
  const fetchData = async () => {
    try {
      ...
      if (err.code === "ERR_NETWORK" || !err.response) {
        setError(
          "Can't reach the server. Make sure the backend is running and reachable at " +
            (api.defaults.baseURL || "the configured API URL") +
            "."
        );
      } else {
        setError(
  ...
  const warningMembers = useMemo(() => {
    return members.map((member) => {
      let warning = 0;
      let overdue = 0;
      const flaggedTasks = []; // FIX #4 support: keep references so the modal can show/act on them
      ...
      allTasks.forEach((task) => {
        ...
        if (assignedId && String(assignedId) === String(member._id)) {
          ...
          // FIX #5: guard against missing/invalid endDate so it doesn't
          // silently produce NaN and skip both buckets without a trace.
          ...
          if (diff < 0) {
            ...
            // NOTE (FIX #6): diff < 0 is already handled above, so this
            // only ever fires when diff === 0 (due today). Change to
            // `diff <= 3` if you want a multi-day "due soon" window.
            warning++;
            flaggedTasks.push(task);
          }
        }
      ...
      return { ...member, warning, overdue, flaggedTasks };
    });
  }, [members, allTasks]);
  ...
  // ---------------- ACTION: TASK UPDATE TRIGGER ----------------
  // FIX #4: this existed before but was never called from the UI. Now wired
  // into the warning-members modal so each flagged task has a working
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="bg-white border border-red-200 rounded-xl p-8 max-w-md text-center shadow-sm">
          ...
          <button
            onClick={fetchData}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-5 py-2 rounded-lg text-sm shadow-sm transition"
          >
            Retry
          </button>
        </div>
              ...
              <div className="space-y-3 overflow-y-auto flex-1 pr-1">
                {flaggedMembers.length === 0 ? (
                  ...
                  flaggedMembers.map((m) => (
                    <div
                      key={m._id}
                      className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-slate-50 transition"
                    >
                      ...
                      {/* FIX #4: flagged tasks listed with a working
                          "Mark complete" button wired to handleCompleteTask */}
                      {m.flaggedTasks.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
                          {m.flaggedTasks.map((task) => (
                            <div
                              key={task._id}
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleCompleteTask(task.projectId, task._id)
                                }
                                disabled={completingTaskId === task._id}
                                className="shrink-0 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white px-2.5 py-1 rounded transition ml-3"
                              >
