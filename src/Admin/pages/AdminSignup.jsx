...
const AdminSignup = () => {
  ...
  // State for sleek visual feedback and button animations
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/user/signup", {
        ...formData,
        role: "admin" // Fixed: Changed from "/admin/login" to standard role string
      });

      setStatus({ type: "success", message: res.data.message || "Admin registered successfully!" });
      
      // Brief delay so the user sees the success message before redirecting
      setTimeout(() => {
        navigate("/admin/login");
      }, 1500);
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-100">
        ...
        <div className="p-8 flex flex-col gap-6">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Create Admin Profile</h1>
            <p className="text-slate-500 text-sm mt-1">Register a new administrator identity</p>
          </div>
