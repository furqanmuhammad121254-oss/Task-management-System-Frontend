...
const Project = () => {
  ...
  // ---------------- FETCH PROJECTS ----------------
  // FIX #1: fetchProjects was called in a useEffect but never defined anywhere,
  // which threw "fetchProjects is not defined" on every mount and prevented
  // the project list from ever loading. Restored the logic that was
  // previously commented out into its own named function.
  const fetchProjects = async () => {
    try {
      const res = await api.get(API_BASE_URL);
      setProjects(res.data);
  ...
  // FIX #5: handleEditTask was mixing project-edit state (formData/showForm)
  // with task data, and was never actually reachable from a task context
  // (the caller passed an undefined `task` variable — see FIX #3 below).
  // This now correctly opens the project popup on the Tasks tab and puts
  // the task into edit mode via `editTask`, which the effect above already
  // uses to hydrate `taskForm`.
  ...
  // ---------------- UI ----------------
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-10 text-slate-800 antialiased">
      <div className="max-w-7xl mx-auto">
        ...
        {/* POPUP OVERLAY */}
        {
          showPop && selectedProject && (
            <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-150">
              <div className="bg-white w-full h-[600px] max-w-4xl rounded-xl shadow-xl border border-slate-100 flex flex-col overflow-hidden max-h-[90vh] animate-in zoom-in-95 duration-200">
                      ...
                      <div className="flex flex-wrap gap-2 overflow-y-auto">
                        {selectedProject?.tasks?.map((task) => {
                          ...
                          return (
                            <div
                              key={task._id}
                              className="flex flex-col justify-between border border-slate-100 rounded-xl p-4 bg-white hover:shadow-md hover:border-slate-200 transition-all duration-200 ease-in-out group w-[380px] h-[220px] ml-3"
                            >
                              <div>
                                {/* Top Row: Task Name & Status Badge */}
                                <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-start">
                      ...
                      <div className="md:col-span-2 bg-slate-50/70 p-4 border border-slate-200/60 rounded-xl space-y-3">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                          {editTask ? "Edit Sprint Task" : "Create Sprint Task"}
                        </h4>
                        ...
                        <div className="flex gap-2">
                          ...
                          {editTask && (
                            <button
                              ...
                              className="w-full font-medium p-2 text-xs rounded-lg shadow-sm transition mt-2 bg-slate-200 hover:bg-slate-300 text-slate-700"
                            >
                              Cancel
                            </button>
                          )}
